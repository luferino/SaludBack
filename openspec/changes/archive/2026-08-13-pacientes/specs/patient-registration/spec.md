...
The create flow MUST record the acting user in `created_by`. The route factory SHALL accept an optional `getActor(req)` hook; without it, or a verified token exposing a `sub` claim, the actor SHALL be `null`. `created_by` MUST equal the actor's id, or NULL when anonymous. Future guards MUST reuse this hook unchanged.
