...
`POST /auth/forgot-password` MUST accept a `username` (empty/missing → HTTP 400). The username lookup MUST normalize to uppercase before searching. Any non-empty username MUST return the same generic HTTP 200 body whether the user has an email, lacks one, or does not exist. A user with an email MUST get a token and a MailerPort-delivered link.
...
- GIVEN a user stored with username `JPEREZ` and a non-NULL email
- WHEN forgot-password sends `username: jperez` (lowercase)
- THEN the lookup matches the stored user
- AND a token is stored and a reset link is mailed
...
`POST /auth/reset-password` MUST accept `token` and `newPassword` (empty/missing → HTTP 400). The `newPassword` MUST meet the password strength rules (min 10 characters, at least one letter and one digit) before the token is consumed; a weak password MUST return HTTP 400 without consuming the token. Unknown, expired, or used tokens MUST be rejected with a generic error without changing the password. Success MUST persist a bcrypt hash (cost default 12) and mark the token used.
...
- GIVEN a valid, unexpired, unused token
- WHEN reset-password sends it with a `newPassword` shorter than 10 characters or lacking a letter or digit
- THEN the system responds 400
- AND the password is unchanged and the token is not consumed
